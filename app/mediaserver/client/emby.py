...
class Emby(_IMediaClient):
    ...
    def __get_emby_series_ids_by_name(self, name, year):
        """
        根据名称查询Emby中剧集的全部SeriesId
        同一部剧集被多个媒体库（或同一媒体库的多个路径）包含时，Emby中会存在多个同名的剧集条目，
        ...
        try:
            ...
            if not res:
                ...
                log.warn(f"【{self.client_name}】emby/Items 未获取到返回数据，无法查询剧集：{name}")
                return None
    ...
    def __get_emby_series_id_by_name(self, name, year):
        """
        根据名称查询Emby中剧集的SeriesId（只返回第一个匹配的条目）
        """
        item_ids = self.__get_emby_series_ids_by_name(name, year)
        if not item_ids:
            return item_ids
        return item_ids[0]
    ...
    def get_tv_episodes(self,
        ...
        同一部剧集在Emby中存在多个同名条目时（比如同时被多个媒体库包含），
        ...
        if item_id:
            ...
            item_ids = self.__get_emby_series_ids_by_name(title, year)
            if item_ids is None:
                return None
            if not item_ids:
                return []
        ...
        for item in item_ids:
            req_url = "%semby/Shows/%s/Episodes?Season=%s&IsMissing=false&api_key=%s" % (
                self._host, item, season, self._apikey)
            try:
